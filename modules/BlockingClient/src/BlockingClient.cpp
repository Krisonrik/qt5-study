...
BlockingClient::BlockingClient(QWidget *parent) : QWidget(parent)
{
  ...
  statusLabel =
      new QLabel(tr("This examples requires that you run the "
  ...
  connect(hostLineEdit,
          ...
          this,
          SLOT(enableGetFortuneButton()));
  connect(portLineEdit,
          SIGNAL(textChanged(QString)),
          this,
          SLOT(enableGetFortuneButton()));
  connect(
      &thread, SIGNAL(newFortune(QString)), this, SLOT(showFortune(QString)));
  connect(&thread,
          SIGNAL(error(int, QString)),
          this,
          SLOT(displayError(int, QString)));
